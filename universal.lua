...
run(function()
	...
	SilentAim = vape.Categories.Combat:CreateModule({
		...
		Function = function(callback)
			if CircleObject then
				CircleObject.Visible = callback and (Mode.Value == 'Mouse' or Mode.Value == 'FixedCircle')
			end
			if callback then
				...
				repeat
					if CircleObject and Mode.Value == 'Mouse' then
						CircleObject.Position = inputService:GetMouseLocation()
					end
	...
	Mode = SilentAim:CreateDropdown({
		Name = 'Mode',
		List = {'Mouse', 'Position', 'FixedCircle'},
		Function = function(val)
			if CircleObject then
				CircleObject.Visible = SilentAim.Enabled and (val == 'Mouse' or val == 'FixedCircle')
			end
		end,
		Tooltip = 'Mouse - Checks for entities near the mouses position\nPosition - Checks for entities near the local character\nFixedCircle - Uses a fixed circle at the center of the screen (for mobile)'
	})
	...
	SilentAim:CreateToggle({
		...
		Function = function(callback)
			if callback then
				...
				CircleObject.Filled = CircleFilled.Enabled
				CircleObject.Color = Color3.fromHSV(CircleColor.Hue, CircleColor.Sat, CircleColor.Value)
				CircleObject.Position = (Mode.Value == 'FixedCircle' and vape.gui.AbsoluteSize / 2) or inputService:GetMouseLocation()
				CircleObject.Radius = Range.Value
				CircleObject.NumSides = 100
				CircleObject.Transparency = 1 - CircleTransparency.Value
				CircleObject.Visible = SilentAim.Enabled and (Mode.Value == 'Mouse' or Mode.Value == 'FixedCircle')
			else
				pcall(function()
					CircleObject.Visible = false
					CircleObject:Remove()
